...
public class OrdenesVinculacionFragment extends Fragment {
    ...
    public void onViewCreated(@NonNull View view, @Nullable Bundle savedInstanceState) {
        ...
        adapter = new OrdenVinculacionAdapter(listaMostrar, new OrdenVinculacionAdapter.OnOrdenClickListener() {
            ...
            @Override
            public void onEdit(OrdenRemito orden) {
                mostrarDialogoEditar(orden);
            }

            @Override
            public void onDelete(OrdenRemito orden) {
    ...
    private void filtrarLista() {
        ...
        for (Remito r : listaRemitos) {
            if (r.getOrdenes() != null) {
                for (OrdenRemito o : r.getOrdenes()) {
                    if (o.isAsociadaAPago() == filtrandoAsociadas) {
                        listaMostrar.add(o);
                    }
                }
            }
        ...
        tvEmpty.setText(filtrandoAsociadas ? "No hay órdenes asociadas a pagos" : "No hay órdenes pendientes de pago");
    }
    ...
    private void mostrarDialogoEditar(OrdenRemito orden) {
        ...
        new AlertDialog.Builder(getContext())
                ...
                .setPositiveButton("Guardar", (d, w) -> {
                    ...
                    try {
                        orden.setCantidadSesiones(Integer.parseInt(etCant.getText().toString()));
                    } catch (Exception e) {}
                    actualizarOrdenEnRemito(orden);
                })
    ...
    private void confirmarEliminacion(OrdenRemito orden) {
        if (orden.isAsociadaAPago()) {
            Toast.makeText(getContext(), "No se puede eliminar una orden asociada a un pago", Toast.LENGTH_SHORT).show();
            return;
        }

        new AlertDialog.Builder(getContext())
                .setTitle("Eliminar Orden")
                .setMessage("¿Estás seguro que deseas eliminar esta orden del remito?")
                .setPositiveButton("Eliminar", (d, w) -> {
                    eliminarOrdenDeRemito(orden);
                })
                .setNegativeButton("Cancelar", null)
                .show();
    }
